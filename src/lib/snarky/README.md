...
> **Note (snarky-rs migration):** this directory used to be the
> `o1-labs/snarky` git submodule. It is now vendored in-tree, trimmed of the
> legacy parts (libsnark backends, `snarky_integer`, `snarky_signature`,
> tests), as part of the migration to the Rust snarky implementation living
> in `src/lib/crypto/proof-systems/snarky`. The goal is to progressively
> replace the modules here with thin shims over the Rust crate — see
> [CLAUDE.md](./CLAUDE.md) for the migration status.
